...
1. Usuario cancela un vuelo desde el panel de cancelación
...
No se requirieron cambios. El backend ya devuelve `vuelo_replanificado_id`, `vuelo_replanificado_codigo` y `plan_viaje` en el response `CancelacionResponse.equipajes` (Java `EquipajeAfectado`).
