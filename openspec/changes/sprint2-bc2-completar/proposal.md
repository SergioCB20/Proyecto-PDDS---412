...
- **Motor de enrutamiento greedy** (`MotorEnrutamiento`): algoritmo stateless que calcula rutas directas o de 2 escalas (mín 60 min conexión), respeta SLA y capacidad de vuelos. Retorna `PlanViaje` con `SegmentoPlan[]`.
- **Replanificación por eventos** (`ReplanificacionService`): escucha `VueloCanceladoEvent`, identifica equipajes afectados, crea lote, ejecuta motor, evalúa SLA breach, actualiza estados.
...
- **Backend BC2**: ~10 archivos nuevos (services, entities, repos, controller, websocket config), ~3 archivos modificados (SesionService, SesionController, RedisCacheService).
- **Backend shared**: 3 nuevos eventos publicados (`PlanViajeCreado`, `ReplanificacionIniciada`, `SesionFinalizada`).
- **Backend BC1**: Integración con eventos publicados por BC2 (consumidor de `PlanViajeCreado` para persistir plan).
- **Frontend**: 2 páginas modificadas (`operacion/page.tsx`, `simulacion/[id]/page.tsx`), posiblemente 1 nueva (`useTelemetria` hook).
- **Redis**: Nuevas claves escritas por BC2 (`sesion:{id}:metricas`, `sesion:{id}:estado`).
- **Dependencias**: WebSocket (spring-boot-starter-websocket) nuevo en backend.
