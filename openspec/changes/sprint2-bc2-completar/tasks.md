...
- [x] 0.1 Crear migración `V18__cola_planificacion.sql` con tabla: id UUID PK, equipaje_id FK → equipajes(id), tipo VARCHAR(20) NOT NULL (PLANIFICACION|REPLANIFICACION), estado VARCHAR(20) NOT NULL DEFAULT 'PENDIENTE', intentos INT DEFAULT 0, error TEXT, fecha_creacion TIMESTAMPTZ DEFAULT NOW(), fecha_procesado TIMESTAMPTZ. Índice en (estado, fecha_creacion).
- [x] 0.2 Crear `ColaPlanificacion.java` entidad JPA en `bc1/domain/` con `@Table(name = "cola_planificacion")`
...
- [x] 0.5 Crear `ColaPlanificacionRepository.java` en `bc1/infrastructure/`:
- [x] 0.6 Crear `PlanificacionWorker.java` en `bc1/application/` con `@Scheduled(fixedDelay = 500)`:
- [x] 0.7 En el mismo `@Scheduled`, ejecutar query de timeout: items EN_PROCESO con fecha_creacion < NOW() - 5 min → marcar FALLIDO con error "Timeout por crash"
...
- [x] 3.1 Crear `MotorEnrutamiento.java` como @Service stateless en `bc2/application/`
- [x] 3.2 Implementar método `calcularRuta(UUID, String, LocalDateTime, UUID)` con algoritmo greedy
- [x] 3.3 Implementar búsqueda de vuelo directo desde nodo origen a destino
...
- [x] 5.3 Implementar avance de `dia_hora_virtual` según factor de escala
...
- [x] 5b.1 Crear `SseService.java` en `shared/infrastructure/` con `ConcurrentHashMap<UUID, SseEmitter>` para gestionar emisores. Métodos: `registrar(UUID sessionId)`, `emitir(UUID sessionId, String evento, Object data)`, `broadcast(String evento, Object data)`, `eliminar(UUID sessionId)`
- [x] 5b.2 Crear `PlanificacionSseController.java` en `bc1/infrastructure/` con `GET /api/eventos/planificacion` que retorna `SseEmitter` (con `@PreAuthorize`)
- [x] 5b.3 El `PlanificacionWorker` notifica a `SseService.broadcast()` cuando completa o falla un item — emite eventos `planificacion-completada` y `planificacion-fallida`
- [x] 5b.4 Manejar timeouts (`onTimeout` → `SseEmitter.complete()`) y desconexiones (`onCompletion` → eliminar del mapa)
- [x] 5b.5 Agregar log cuando no hay emisores conectados (no enviar notificación si nadie escucha)

## 6. RedisCacheService - métodos faltantes

- [x] 6.1 Agregar método `setMetricasSesion(UUID, String)` para escribir JSON en `sesion:{id}:metricas`
- [x] 6.2 Agregar método `getMetricasSesion(UUID)` para leer JSON desde `sesion:{id}:metricas`
- [x] 6.3 Agregar método `setEstadoSesion(UUID, String)` para escribir en `sesion:{id}:estado`
- [x] 6.4 Agregar método `getEstadoSesion(UUID)` para leer desde `sesion:{id}:estado`

## 7. WebSocket de telemetría (B9)

- [x] 7.1 Agregar dependencia `spring-boot-starter-websocket` en `pom.xml`
- [x] 7.2 Crear `WebSocketConfig.java` con `@EnableWebSocket` y registro de handler
- [x] 7.3 Crear `TelemetriaWebSocket.java` extending `TextWebSocketHandler`
- [x] 7.4 Implementar autenticación por query param `?token={jwt}` en `afterConnectionEstablished`
- [x] 7.5 Implementar emisión de JSON de telemetría cada tick (nodos, vuelos, metricas_sesion) via TelemetriaService
- [x] 7.6 Implementar interpolación de posición de vuelos EN_RUTA (lat/lon) via TelemetriaService
- [x] 7.7 Implementar cálculo de `ocupacion_pct` y `color` para nodos via TelemetriaService
- [x] 7.8 Implementar cálculo de `ocupacion_pct` y `color` para vuelos via TelemetriaService
- [x] 7.9 Manejar desconexiones y limpieza de sesiones WebSocket
